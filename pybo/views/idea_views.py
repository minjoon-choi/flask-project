...
def _list():
    ...
    if so == "recommend":
        sub_query = (
            db.session.query(idea_voter.c.idea_id, func.count("*").label("num_voter"))
            .group_by(idea_voter.c.idea_id)
            .subquery()
        )
        idea_list = Idea.query.outerjoin(
            sub_query, Idea.id == sub_query.c.idea_id
        ).order_by(_nullslast(sub_query.c.num_voter.desc()), Idea.regDate.desc())
    elif so == "popular":
        sub_query = (
            db.session.query(Feedback.idea_id, func.count("*").label("num_feedback"))
            .group_by(Feedback.idea_id)
            .subquery()
        )
        idea_list = Idea.query.outerjoin(
            sub_query, Idea.id == sub_query.c.idea_id
    ...
    if kw:
        search = "%%{}%%".format(kw)
        sub_query = (
            db.session.query(Feedback.idea_id, Feedback.content, User.username)
        ...
        idea_list = (
            ...
            .filter(
                Idea.ideaTitle.ilike(search)
                | Idea.prodID.ilike(search)  # 아이디어 제목
                | Idea.content.ilike(search)  # 계열사품목코드
                | User.username.ilike(search)  # 제안내용
    ...
    return render_template(
        "idea/idea_list.html", idea_list=idea_list, page=page, kw=kw, so=so
    )
